...
const FormDropdown = ({title, options, value, changeHandler, paddingTop, paddingBottom}: Props) => {
  return (
    <div className={classNames('flex', 'flex-col', 'px-10', paddingTop, paddingBottom, 'font-bold')}>
      ...
      <select onChange={(event: React.ChangeEvent<HTMLSelectElement>) => changeHandler(event.currentTarget.value)} value={value} className="p-3 rounded-lg text-lg focus:outline-none focus:outline-orange-500">
